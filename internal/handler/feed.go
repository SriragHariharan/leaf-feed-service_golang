...
func (h *FeedHandler) GetFeed(w http.ResponseWriter, r *http.Request) {
	...
	if userID == "" {
		writeError(w, http.StatusBadRequest, "invalid_request", "query param user_id is required")
		return
	}
...
func (h *FeedHandler) GetTimeline(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeError(w, http.StatusInternalServerError, "internal_error", "failed to fetch timeline")
		if strings.Contains(strings.ToLower(err.Error()), "decode cursor") || errors.Is(err, context.DeadlineExceeded) {
